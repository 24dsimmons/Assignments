...
console.log(averageTemp(weeklyTemps))

// Personal Notes for Further Reference:
// Arrow Functions {=>}
// (parameter => action) (input => output)
...
console.log(topCSStudents(students))

// TODO 2c: Build a grade report using reduce().
// Return: { highest: ..., lowest: ..., average: ... }
function gradeReport(arr) {
const highest = Math.max(...arr.map(student => student.grade))
const lowest = Math.min(...arr.map(student => student.grade))
const average = arr.reduce((sum, student) => sum + student.grade, 0) / arr.length;
